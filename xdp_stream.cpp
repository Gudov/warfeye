#define ENABLE_PIPEWIRE_FEATURE_1
...
static struct {
    ...
    size_t w,h;
    std::function<void(void*,uint32_t,size_t,size_t)> callback;
} global;
...
static void on_stream_process(void *userdata)
{
    ...
    global.callback(buf->datas[0].data, buf->datas[0].chunk->size, global.w, global.h);
...
static void on_stream_param_changed(void *userdata, uint32_t id, const struct spa_pod *param)
{
    if (id == SPA_PARAM_Format) {
        ...
        global.w = format.info.raw.size.width;
        global.h = format.info.raw.size.height;
...
void start_pipewire(guint32 node_id)
{
    ...
    auto rect_0 = SPA_RECTANGLE(320, 240);
    auto rect_1 = SPA_RECTANGLE(1, 1);
    auto rect_2 = SPA_RECTANGLE(4096, 4096);
    auto frac_0 = SPA_FRACTION(25, 1);
    auto frac_1 = SPA_FRACTION(0, 1);
    auto frac_2 = SPA_FRACTION(1000, 1);
...
static void on_xdp_session_start(GObject *obj, GAsyncResult *result, void *user_data) {
...

    {
        ...
        while (g_variant_iter_loop(streams_iter, "(ua{sv})", &pipewire_node_id, &property_iter)) {
            logf("stream pipewire_node_id=%u", pipewire_node_id);
            last_valid_id = pipewire_node_id;
            stream_count++;
...
void init_screencast(int argc, char *argv[], std::function<void(void*,uint32_t,size_t,size_t)> callback)
{
    ...
    XdpOutputType output_type = XdpOutputType(XDP_OUTPUT_MONITOR | XDP_OUTPUT_WINDOW | XDP_OUTPUT_VIRTUAL);
